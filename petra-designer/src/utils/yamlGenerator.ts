...
export function generateYaml(nodes: Node[], edges: Edge[]): string {
  ...
  nodes
    ...
    .forEach((n, index) => {
      ...
      const baseName = data.label || `signal_${index}`
  ...
  nodes
    ...
    .forEach((n, index) => {
      ...
      const blockName = (data.label || `block_${index}`).toLowerCase().replace(/[^a-z0-9_]/g, '_')
        ...
        .forEach(e => {
          ...
          if (sourceNode) {
            const signalName = signalNameMap.get(sourceNode.id) || 
                             sourceNode.data.label?.toLowerCase().replace(/[^a-z0-9_]/g, '_') || 
                             'unnamed'
       ...
       .forEach(e => {
         ...
         if (targetNode) {
           const signalName = signalNameMap.get(targetNode.id) || 
                            targetNode.data.label?.toLowerCase().replace(/[^a-z0-9_]/g, '_') || 
                            'unnamed'
 ...
 if (twilioNodes.length > 0) {
   config.twilio = {
     ...
     actions: twilioNodes.map((n, index) => {
       ...
       const name = (data.label || `twilio_${index}`).toLowerCase().replace(/[^a-z0-9_]/g, '_')
       
       // Find trigger signal
       const triggerEdge = edges.find(e => e.target === n.id)
       const triggerNode = triggerEdge ? nodes.find(m => m.id === triggerEdge.source) : null
       const triggerSignal = triggerNode ? 
         (signalNameMap.get(triggerNode.id) || triggerNode.data.label?.toLowerCase().replace(/[^a-z0-9_]/g, '_')) : 
         'unknown_trigger'
